...
| ----------- | ----------------------- | ------------------ | --------------------------------------------------------- |
| 400         | `policy_violation`      | `content_blocked`  | An inbound rule blocked the user's message.               |
| 400         | `policy_violation`      | `response_blocked` | An outbound rule blocked the model's response.            |
...
### Inbound Block (content\_blocked)

This occurs when a user's message violates an inbound rule:
...
### Outbound Block (response\_blocked)

This occurs when the model's response violates an outbound rule:
...
def send_message(user_input: str) -> str:
    try:
        ...
        if e.code == "content_blocked":
            # Inbound rule blocked the user's message
            return "Your message was blocked by a security policy."

        if e.code == "response_blocked":
            # Outbound rule blocked the model's response
...
Errors can occur mid-stream when outbound rules block a response. Wrap your streaming loop in a try/except:
...
const client = new OpenAI({
  baseURL: "https://api.collieai.com/v1",
...
3. For streaming with outbound rules, the first chunk is delayed while the full response is accumulated and filtered. Set a higher timeout for streaming requests.
...
3. If the stream terminates early with an error, check whether an outbound rule blocked the response. See [Streaming](/broken/pages/b3d45e7b0803399b1834e5f260aa414c6a40075b) for details on how outbound filtering interacts with streaming.
