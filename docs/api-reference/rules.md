...
Rules define individual checks within a policy. Each rule evaluates inbound messages, outbound messages, or both, and determines whether to allow, block, or modify content.
...
| `direction`        | string  | Yes      | When to evaluate: `inbound`, `outbound`, or `both`             |
...
**URL:** `https://api.collieai.com/api/v1/policies/{policy_id}/rules`
...
```bash
curl https://api.collieai.com/api/v1/policies/pol_abc123/rules \
...
```bash
curl -X POST https://api.collieai.com/api/v1/policies/pol_abc123/rules \
  -H "Authorization: Bearer <token>" \
...
**URL:** `https://api.collieai.com/api/v1/policies/{policy_id}/rules/{rule_id}`
...
```bash
curl -X PATCH https://api.collieai.com/api/v1/policies/pol_abc123/rules/rule_003 \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "is_enabled": false,
...
```bash
curl -X DELETE https://api.collieai.com/api/v1/policies/pol_abc123/rules/rule_003 \
  -H "Authorization: Bearer <token>"
```
...
**URL:** `https://api.collieai.com/api/v1/policies/{policy_id}/rules/{rule_id}/test`
...
| `direction` | string | No       | Test direction: `inbound` or `outbound`   |

### Example Request

```bash
curl -X POST https://api.collieai.com/api/v1/policies/pol_abc123/rules/rule_001/test \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
