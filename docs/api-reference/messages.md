...
**URL:** `https://api.collieai.com/v1/messages`
...
```bash
curl -X POST https://api.collieai.com/v1/messages \
...
Native `/v1/messages` accepts text content only (see [v1 restrictions](#v1-restrictions)) — both string-shaped and `[{"type": "text", "text": "..."}]`-list-shaped content. Inbound filtering runs on each message's text content; outbound filtering runs on the assembled response text before it reaches the client. If a rule masks content, the masked text is emitted in a single synthetic text block; the original token counts from upstream are preserved in the response so client-side billing reconciliation matches Anthropic's invoice.
