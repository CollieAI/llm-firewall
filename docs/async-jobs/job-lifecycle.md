...
{% hint style="info" %}
In combined mode (both `message_input` and `message_output` provided), the job skips `awaiting_response` and moves directly from inbound to outbound processing.
{% endhint %}
...
| Status                | Description                                                                                                      |
| --------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `processing_inbound`  | Inbound content is being filtered against your rules.                                                            |
| `inbound_blocked`     | Inbound content was blocked by a policy rule. **Terminal.**                                                      |
| `awaiting_response`   | Inbound filtering is complete. Waiting for you to submit the LLM response via `POST /v1/jobs/{job_id}/response`. |
| `processing_outbound` | Outbound content (LLM response) is being filtered.                                                               |
| `outbound_blocked`    | Outbound content was blocked by a policy rule. **Terminal.**                                                     |
...
```bash
curl https://api.collieai.com/api/v1/health/queues \
  -H "Authorization: Bearer clai_xxx"
```
...
{% step %}
### Use separate inbound and outbound fields for best ML accuracy

Prefer `message_input` and `message_output` over the legacy `message` field. CollieAI applies different rule sets to inbound and outbound content. Using the correct fields gives the ML models better context and improves filtering accuracy.
{% endstep %}
{% endstepper %}
