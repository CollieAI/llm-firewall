...
| Event | Description |
|-------|-------------|
| `job.inbound_complete` | Inbound filtering finished. The filtered content is in `data.filtered`. |
| `job.inbound_blocked` | Inbound content was blocked by a policy rule. |
| `job.outbound_complete` | Outbound filtering finished. The filtered response is in `data.filtered`. |
| `job.outbound_blocked` | Outbound content was blocked by a policy rule. |

---

## Payload Format

### job.inbound\_complete
