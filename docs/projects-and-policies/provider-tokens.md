...
```bash
curl -X PATCH https://api.collieai.com/api/v1/tokens/{token_id} \
  -H "Authorization: Bearer clai_..." \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X DELETE https://api.collieai.com/api/v1/tokens/{token_id} \
  -H "Authorization: Bearer clai_..."
```
